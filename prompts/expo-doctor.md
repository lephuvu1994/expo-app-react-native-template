...
1. Run expo doctor command using `yarn run doctor`
...
5. If the issues is fixed, make sure to commit changes for package.json and yarn-lock.yaml with the message `git add package.json yarn-lock.yaml && git commit -m "fix(deps): expo doctor issues"`
